...
def create_fmt_csv() :
  ...
  with open('parameter_collect/{}.csv'.format(parameter_split_list[0][0][0]), 'w') as f:
  ...
  parameter_fmt = open('parameter_collect/FMT.csv')
...
def create_parameter_csv() :
  ...
  for i in range(1, len(parameter_split_list)) :
    ...
    with open('parameter_collect/{}.csv'.format(parameter_split_list[i][0][0]), 'w') as f:
...
def get_battery_time() :
  ...
  file = open('parameter_collect/BAT.csv')
...
def color_of_energy_consumption() :
  ...
  get_mean = np.mean(voltcurr_list)

  # Calculate the standard deviation
  get_sd = np.std(voltcurr_list)

  for data in voltcurr_list :
    if data < get_mean - (2 * get_sd) :
      color_path.append('dark green')
    elif data > get_mean - (2 * get_sd) and data < get_mean - get_sd :
      color_path.append('green')
    elif data > get_mean - get_sd and data < get_mean + get_sd :
      color_path.append('yellow')
    elif data > get_mean + get_sd and data < get_mean + (2 * get_sd) :
      color_path.append('orange')
    elif data > get_mean + (2 * get_sd) :
      color_path.append('red')
  
  print('mean is {} and SD is {}'.format(get_mean, get_sd))
...
for i in range(len(parameter_array)) :
  file = open('parameter_collect/{}.csv'.format(parameter_array[i]))
...
with open('data_to_web/Complete_data.csv', 'w') as f:
